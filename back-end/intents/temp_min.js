...
async function handleTempMinIntent(cidade, consultation, res, queryText) {
    if (!cidade) {
        return res.json({ "fulfillmentText": "Por favor, forneça o nome da cidade para obter a previsão de temperatura mínima." });
    }
    try {
        ...
        const resposta = `A temperatura mínima prevista para ${currentWeather.name} é de ${tempMin}°C.`;
...
module.exports = handleTempMinIntent;
