...
const LoginOrRegister = () => {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        const response = await fetch('https://atmos-chatbot-climatico.vercel.app/api/auth/login', {
          method: 'POST',
          headers: {
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        const response = await fetch('https://atmos-chatbot-climatico.vercel.app/api/auth/register', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
